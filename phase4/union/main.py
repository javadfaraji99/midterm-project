...
def union_nfa_machines(nfa1_file_path, nfa2_file_path):
    ...
    s = 'q' + str(number_of_nfa1_states + number_of_nfa2_states)
    s1 = 'q' + str(number_of_nfa1_states + number_of_nfa2_states + 1)

    # Create a new NFA machine by concatenating the two machines
    new_nfa = {
        "states": nfa1["states"] | nfa2["states"] | {s,s1},
    ...
    for state in nfa1["final_states"]:
        new_nfa["transitions"][state][""] = {s1}

    for state in nfa2["final_states"]:
        new_nfa["transitions"][state][""] = {s1}

    transition = {"" : {nfa1["initial_state"],nfa2["initial_state"]}}
...
new_nfa = union_nfa_machines(nfa1_file_path, nfa2_file_path)
...
with open('union_output.json','w') as f:
    f.write(outfile)
